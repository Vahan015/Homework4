...
fun WelcomeScreen(navController: NavController, activity: MainActivity, viewModel: WeatherViewModel) {
    ...
    weatherData?.let { data ->
        Text(text = "Weather in ${data.cityName}", style = MaterialTheme.typography.h6)
...
private fun fetchWeatherForCurrentLocation(activity: MainActivity, viewModel: WeatherViewModel) {
    ...
    fun LocationDisplay(location: String) {
        Text(text = "Your current location: $location", style = MaterialTheme.typography.subtitle1)
    }


    @Composable
    fun WeatherInfoDisplay(weatherData: WeatherData, isFahrenheit: Boolean) {
        val displayTemp = convertTemperature(weatherData.temperature, isFahrenheit)
        Column {
            Text(text = "Weather Information", style = MaterialTheme.typography.h6)
            Spacer(modifier = Modifier.height(8.dp))
            Text(text = "Temperature: ${displayTemp}°${if (isFahrenheit) "F" else "C"}")
            Text(text = "Humidity: ${weatherData.humidity ?: "N/A"}%")
        }
    }
